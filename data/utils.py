...
def adjust_oxygen_pos_grad(
    atom_37: torch.Tensor, pos_is_known = None
) -> torch.Tensor:
    """
    Gradient-friendly version of adjust_oxygen_pos that avoids in-place operations.

    Imputes the position of the oxygen atom on the backbone by using adjacent frame information.
    Uses out-of-place operations to preserve gradient flow.
    ...
    Returns:
        New tensor with adjusted oxygen positions (does not modify input).
    """
    N = atom_37.shape[0]
    assert atom_37.shape == (N, 37, 3)
    ...
    calpha_to_carbonyl = calpha_to_carbonyl / (
        torch.sqrt((calpha_to_carbonyl ** 2).sum(dim=-1, keepdim=True) + 1e-12)
    )

    # Nitrogen of the next frame to carbonyl of the current frame.
    nitrogen_to_carbonyl = (atom_37[:-1, 2, :] - atom_37[1:, 0, :])
    nitrogen_to_carbonyl = nitrogen_to_carbonyl / (
        torch.sqrt((nitrogen_to_carbonyl ** 2).sum(dim=-1, keepdim=True) + 1e-12)
    )

    carbonyl_to_oxygen = calpha_to_carbonyl + nitrogen_to_carbonyl
    carbonyl_to_oxygen = carbonyl_to_oxygen / (
        torch.sqrt((carbonyl_to_oxygen ** 2).sum(dim=-1, keepdim=True) + 1e-12)
    )

    # Compute oxygen positions for residues with known next residue
    oxygen_pos_main = atom_37[:-1, 2, :] + carbonyl_to_oxygen * 1.23  # (N-1, 3)

    # For the last residue (C-terminus), use alternative calculation
    calpha_to_carbonyl_term = (atom_37[-1:, 2, :] - atom_37[-1:, 1, :])
    calpha_to_carbonyl_term = calpha_to_carbonyl_term / (
        torch.sqrt((calpha_to_carbonyl_term ** 2).sum(dim=-1, keepdim=True) + 1e-12)
    )
    calpha_to_nitrogen_term = (atom_37[-1:, 0, :] - atom_37[-1:, 1, :])
    calpha_to_nitrogen_term = calpha_to_nitrogen_term / (
        torch.sqrt((calpha_to_nitrogen_term ** 2).sum(dim=-1, keepdim=True) + 1e-12)
    )
    carbonyl_to_oxygen_term = calpha_to_carbonyl_term + calpha_to_nitrogen_term
    carbonyl_to_oxygen_term = carbonyl_to_oxygen_term / (
        torch.sqrt((carbonyl_to_oxygen_term ** 2).sum(dim=-1, keepdim=True) + 1e-12)
    )
    oxygen_pos_term = atom_37[-1:, 2, :] + carbonyl_to_oxygen_term * 1.23  # (1, 3)

    # Combine oxygen positions
    new_oxygen_pos = torch.cat([oxygen_pos_main, oxygen_pos_term], dim=0)  # (N, 3)

    # Handle pos_is_known masking if provided
    if pos_is_known is not None:
        # For residues where next is not known, use terminal calculation
        next_res_gone = ~pos_is_known.bool()
        next_res_gone = torch.cat(
            [next_res_gone, torch.ones((1,), device=pos_is_known.device).bool()], dim=0
        )[1:]  # (N,)

        # Compute terminal-style oxygen for all residues
        calpha_to_carbonyl_all = (atom_37[:, 2, :] - atom_37[:, 1, :])
        calpha_to_carbonyl_all = calpha_to_carbonyl_all / (
            torch.sqrt((calpha_to_carbonyl_all ** 2).sum(dim=-1, keepdim=True) + 1e-12)
        )
        calpha_to_nitrogen_all = (atom_37[:, 0, :] - atom_37[:, 1, :])
        calpha_to_nitrogen_all = calpha_to_nitrogen_all / (
            torch.sqrt((calpha_to_nitrogen_all ** 2).sum(dim=-1, keepdim=True) + 1e-12)
        )
        carbonyl_to_oxygen_all = calpha_to_carbonyl_all + calpha_to_nitrogen_all
        carbonyl_to_oxygen_all = carbonyl_to_oxygen_all / (
            torch.sqrt((carbonyl_to_oxygen_all ** 2).sum(dim=-1, keepdim=True) + 1e-12)
        )
        oxygen_pos_alt = atom_37[:, 2, :] + carbonyl_to_oxygen_all * 1.23

        # Blend using soft mask for gradient flow
        mask = next_res_gone.float().unsqueeze(-1)  # (N, 1)
        new_oxygen_pos = (1 - mask) * new_oxygen_pos + mask * oxygen_pos_alt

    # Build output tensor using cat (not in-place assignment)
    # atom37 order: N(0), CA(1), C(2), CB(3), O(4), rest(5-36)
    result = torch.cat([
        atom_37[:, :4, :],              # N, CA, C, CB (indices 0-3)
        new_oxygen_pos.unsqueeze(1),    # O (index 4)
        atom_37[:, 5:, :],              # Rest (indices 5-36)
    ], dim=1)
