...
def skew(omega: torch.Tensor) -> torch.Tensor:
    """skew-symmetric part: 0.5*(omega - omega^T)"""
    return 0.5 * (omega - omega.transpose(-1, -2))


def project_to_tangent_SO3(rotmat: torch.Tensor, v: torch.Tensor) -> torch.Tensor:
    """
    Tangential projection of an ambient matrix v onto T_R SO(3),
    using the Frobenius inner product in the ambient space.

    rotmat: (..., 3, 3) orthogonal with det=1
    v: (..., 3, 3) ambient direction (e.g., Euclidean gradient)
    Returns:
        (..., 3, 3) tangent matrix in T_R SO(3)
    """

    # This part transforms to the lie algebra space so(3)
    Rtv = rotmat.transpose(-1, -2) @ v

    # Projection onto so(3)
    Omega = skew(Rtv)

    # Transform back to the group space SO(3)
    return rotmat @ Omega


def project_to_so3(rotmat: torch.Tensor, v: torch.Tensor) -> torch.Tensor:
    """
    Tangential projection of an ambient matrix v onto so(3).

    rotmat: (..., 3, 3) orthogonal with det=1
    v: (..., 3, 3) ambient direction (e.g., Euclidean gradient)
    Returns:
        (..., 3) rotation vector representation in so(3)
    """

    # This part transforms to the lie algebra space so(3)
    Rtv = rotmat.transpose(-1, -2) @ v

    # Projection onto skew-symmetric matrix representation of so(3)
    Omega = skew(Rtv)

    # Convert to rotation vector representation in so(3)
    return skew_matrix_to_vector(Omega)
