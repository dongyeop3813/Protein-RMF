...
def compute_backbone(bb_rigids, psi_torsions, differentiable=False):
    ...
    if differentiable:
        # Use cat instead of in-place assignment for gradient flow
        # Reorder: [N, CA, C] from 0:3, [CB] from 4, [O] from 3, then zeros for rest
        padding = torch.zeros(
            atom14_pos.shape[:-2] + (32, 3),
            device=bb_rigids.device,
            dtype=atom14_pos.dtype
        )
        atom37_bb_pos = torch.cat([
            atom14_pos[..., :3, :],   # N, CA, C (indices 0, 1, 2)
            atom14_pos[..., 4:5, :],  # CB (index 3)
            atom14_pos[..., 3:4, :],  # O (index 4)
            padding,                   # Remaining atoms (indices 5-36)
        ], dim=-2)
    else:
...
def transrot_to_atom37_grad(transrot_traj, res_mask):
    """Gradient-friendly version that avoids in-place operations."""
    atom37_traj = []
    ...
    for trans, rots in transrot_traj:
        ...
        atom37 = compute_backbone(
            ...
            differentiable=True  # Use gradient-friendly path
        )[0]
        batch_atom37 = []
        for i in range(num_batch):
            batch_atom37.append(
                du.adjust_oxygen_pos_grad(atom37[i], res_mask[i])  # Use gradient-friendly version
            )
        atom37_traj.append(torch.stack(batch_atom37))
    atom37_traj = torch.cat(atom37_traj, dim=0)  # Use torch.cat instead of concatenate
    return atom37_traj
