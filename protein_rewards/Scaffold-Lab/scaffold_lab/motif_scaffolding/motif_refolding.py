...
class MotifEvaluator:
    ...
    def _evaluate_diversity(
        ...
        if os.path.exists(diversity_result_path):
            ...
            for pdb in cluster_centers:
                ...
                pdb_name = pdb if pdb.endswith('.pdb') else f"{pdb}.pdb"
                old_path = os.path.join(successful_backbone_dir, pdb_name)
    ...
    def _evaluate_novelty(
        ...
        if os.listdir(successful_backbone_dir):
            ...
            novelty_lookup = dict(zip(unique_backbones["backbone_path"].apply(lambda x: Path(x).stem), unique_backbones["pdbTM"]))
